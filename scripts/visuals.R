...
# Save Plot
png("../ani_mash_visuals/Scatter_plot.png", width = 800, height = 600)  # Open graphics device
# Make Plot
...
# Save Plot
png("../ani_mash_visuals/ani_boxplot_plot.png", width = 800, height = 600)  # Open graphics device
# Make plot 
ggplot(combined_df, aes(x = factor(coverage), y = ani, fill = factor(coverage))) +
  geom_boxplot() +
  labs(x = "Sequence Depth", y = "ANI Score") +
  theme_minimal() +
  theme(
    axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16),
...
# Save Plot
png("../ani_mash_visuals/mash_boxplot_plot.png", width = 800, height = 600)  # Open graphics device
# Make plot 
ggplot(combined_df, aes(x = factor(coverage), y = mash, fill = factor(coverage))) +
  geom_boxplot() +
  labs(x = "Sequence Depth", y = "mash Score") +
  theme_minimal() +
  theme(
    axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16),
...
# Save Plot
png("../ani_mash_visuals/faceted_scatter_plot.png", width = 800, height = 600)  # Open graphics device
